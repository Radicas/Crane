...
double calculateArcAngle(const POINT& p1, const POINT& p2, const POINT& op, bool clockwise) {
    double include_angle = sweep_angle(p1, p2, op);
    double multiply      = crossmultiply(p1, p2, op);
    if (multiply > 0) {
        if (clockwise) {
            return 360 - include_angle;
        }
        else {
...
std::vector<geometry::POINT> segArcIntersectPoints(const geometry::POINT& A, const geometry::POINT& B,
                                                   const geometry::POINT& C, double radius) {
    ...
    double c = pow(x1, 2) + pow(y1, 2) + pow(cx, 2) + pow(cy, 2) - 2 * (x1 * cx + y1 * cy) - pow(radius, 2);
    ...
    if (discriminant < 0) {
        ...
        std::cout << "交点坐标：(" << px << ", " << py << ")" << std::endl;
        res.emplace_back(px, py);
    }
    else {
        // 判别式大于0，有两个交点
        ...
        if (t1 >= 0 && t1 <= 1) {
            ...
            double py1 = y1 + t1 * (y2 - y1);
            std::cout << "交点坐标：(" << px1 << ", " << py1 << ")" << std::endl;
            res.emplace_back(px1, py1);
        }

        if (t2 >= 0 && t2 <= 1) {
            double px2 = x1 + t2 * (x2 - x1);
            double py2 = y1 + t2 * (y2 - y1);
            res.emplace_back(px2, py2);
            std::cout << "交点坐标：(" << px2 << ", " << py2 << ")" << std::endl;
        }
    }
    return res;
}
